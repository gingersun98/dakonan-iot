...
public class GameManager : MonoBehaviour
{
    ...
    public CinemachineImpulseSource exploseSource;
    ...
    [HideInInspector] public Vector3 phoneOffset;
    ...
    private void Update()
    {
        ...
        // Mouse (works in Editor + some desktop builds)
    ...
    public IEnumerator StartPlayerTurn(BallContainer containerToTake, bool hasSpunBefore)
    {
        ...
        // MOVE ALL TO INVENTORY FIRST (NO DELAY)
        foreach (GameObject ball in activeBalls)
        {
            MoveToTarget(
                ball.transform,
    ...
    public IEnumerator StartEnemyTurn(BallContainer containerToTake, bool hasSpunBefore)
    {
        ...
        while (i < activeBalls.Count)
        {
            ...
            else
            {
                if (holeIndexProgress >= playerHoles.Count)
                {
                    // PLAYER BASE (SKIPPED LOGIC IF NEEDED LATER)
        ...
        if (currentTarget.isPlayerSide == false)
        {
            ...
            // mirror index because board is reversed
            int mirroredIndex = enemyHoles.Count - 1 - index;

            BallContainer opposite = playerHoles[mirroredIndex];

            // ✔ CAPTURE RULE (enemy side empty + has spin + opposite not empty)
        ...
        else
        {
            ...
            // ❌ PLAYER EMPTY HOLE = enemy loses turn
            if (wasEmpty)
            {
                SwitchPlayerTurn();
                yield break;
    ...
    {
        // If any hole still contains balls, game continues
        foreach (BallContainer hole in playerHoles)
        {
            if (hole.CheckBallCount() > 0)
                return false;
    ...
    private IEnumerator CountWinningBall(bool isPlayer)
    {
        ...
        confettiParticle.transform.position =
            winnerBase.transform.position;
    ...
    private IEnumerator CaptureToBase(GameObject[] balls, BallContainer baseContainer)
    {
        ...
        // optional but IMPORTANT: finalize insertion after movement
